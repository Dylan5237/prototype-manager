...
- 16077 当前 release 为 `20260901-190258-3d81476a`，已通过 release/manifest、PM2/Nginx、认证 API、分页/筛选回读和目标项目 iframe 浏览器复测；16088 未进行本轮发布。
...
- 阶段 22 BL-007 管理员全量原型只读预览已完成平台实现、本地/API 回归和 16077 跨用户真实复核；项目模块已确认固定打开第一个已绑定菜单，并完成列表摘要/筛选分页、绑定原型服务端搜索分页、MCP 签出门禁和预览 ResizeObserver 误报修复，目标项目 iframe 真实浏览器复测通过；完整阶段验收仍见 [`docs/PHASE22_PROJECT_MODULE_EVIDENCE.md`](docs/PHASE22_PROJECT_MODULE_EVIDENCE.md)。
