...
- 16077 当前 release 为 `20260901-182503-da787bef`，已通过 release/manifest、PM2/Nginx、认证 API、分页/筛选回读和 10 次只读性能基线；新浏览器页面验收尚未执行，16088 未进行本轮发布。
...
- 阶段 22 BL-007 管理员全量原型只读预览已完成平台实现、本地/API 回归和 16077 跨用户真实复核；项目模块已确认固定打开第一个已绑定菜单，并完成列表摘要/筛选分页、绑定原型服务端搜索分页和 MCP 签出门禁修复，详见 [`docs/PHASE22_PROJECT_MODULE_EVIDENCE.md`](docs/PHASE22_PROJECT_MODULE_EVIDENCE.md)。
