...
const ONBOARDING_LAUNCHER_SOURCE = [
  ...
  "async function load(){for(let i=0;i<3;i++){try{const a=new AbortController();var t=setTimeout(()=>a.abort(),15000),r=await fetch(u,{signal:a.signal});if(!r.ok)throw fail('ONBOARDING_DOWNLOAD_HTTP_'+r.status,'Onboarding HTTP '+r.status,r.status===408||r.status===429||r.status>=500);const b=Buffer.from(await r.arrayBuffer());if(crypto.createHash('sha256').update(b).digest('hex')!==s.toLowerCase())throw fail('ONBOARDING_DIGEST_MISMATCH','Onboarding SHA-256 mismatch');return b}catch(e){if(e.name==='AbortError')e=fail('ONBOARDING_DOWNLOAD_TIMEOUT','Onboarding download timed out',true);if(i===2||!(e.retryable||e.name==='TypeError'||e.code==='ECONNRESET'||e.code==='ETIMEDOUT'))throw e;await new Promise(r=>setTimeout(r,250*(i+1)))}finally{clearTimeout(t)}}}",
