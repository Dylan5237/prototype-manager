...
2. 使用 Authorization: Bearer <短期 token> 下载两个 ZIP；禁止把 token、ZIP 或解压内容写入业务仓库。
...
4. 将 MCP ZIP 解压到客户端本地工具目录，确认 Node.js >=18，并用 stdio 接入：
   ...
   env: { FUXI_API_URL: "${baseUrl}", FUXI_TOKEN: "${token}" }
5. 调用 fuxi-platform 的 check_connection，确认 tools/list 包含 deliver_project，再确认 ${SKILL_NAME} 可被调用。
6. 成功后只报告安装位置、连接结果、Skill 状态和 token 过期时间，不回显完整 token。

恢复规则：需要客户端授权时只请求最小原生授权；权限拒绝报告 AUTHORIZATION_REQUIRED；token 过期报告 AUTHENTICATION_FAILED 并让我回平台重新生成；任一步失败都不得声称接入成功。`;

  res.json({
    success: true,
    data: { prompt, token, expiresIn, expiresAt, skillName: SKILL_NAME, skillUrl, mcpUrl, apiUrl: baseUrl }
  });
});
