...
3. For the normal 16077 loop, run `scripts/deploy-test-from-gitlab.ps1 -ConfirmTestDeploy DEPLOY_FUXI_TEST` (or the compatibility wrapper `quick-deploy-test.ps1`). It fresh-clones both platform and Skill `develop` from GitLab, builds with `-Lightweight`, and deploys the immutable archive. Never package a feature worktree directly for 16077.
...
- `deploy-test-from-gitlab.ps1`: fresh-clone both platform and Skill GitLab `develop`, build with the lightweight profile, and delegate deployment to isolated `16077`.
