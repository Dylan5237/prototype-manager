...
目标：
- 项目成员不接触 Git、分支、MR 或代码托管凭据，也能把 AI 修改提交为独立候选。
- 候选上传不改变当前原型；Owner/Admin 预览后显式采用才生成正式版本。
- 采用以基础版本 CAS 防止旧候选静默覆盖新版本，冲突时标记 `stale`，不自动合并。

已完成：
- 新增轻协作设计包，冻结“任务码 → 候选 ZIP → 独立预览 → 采用/退回 → 正式版本”的纵向切片；旧 Git/GitLab 方案标记为历史方案。
- 后端增加增量字段、一次性任务、候选独立存储、审核状态、文件交换补偿、正式版本生成和审计事件。
- 项目页增加“让 AI 修改”、任务提示词、候选列表/预览、采用、退回和过期恢复文案。
- MCP 增加 `create_change_handoff`、`redeem_change_handoff`、`get_change_status`、`submit_change_candidate`，工具总数由 22 增至 26。

本地验收证据：
- 后端 `npm test`：31/31 通过，新增完整提示词、编辑轮换任务码、领取后锁定、删除撤销任务码回归。
- MCP `npm run check` 与 `npm run test:integration` 通过；隔离后端真实执行 26 工具，轻协作创建/兑换/重放拒绝/上传/预览/采用/回读为 `verified`。
- 前端 `npm run build` 通过；设计包严格校验 0 error / 0 warning；浏览器复验正常、采用与过期恢复路径，修复隐藏态同时显示问题。

测试环境部署与真实验收证据（2026-08-20）：
- 不可变 release：`20260820-115227-8ee2d198`；平台 commit `8ee2d198a72353366503d57943233bead1d419a2`；归档 SHA-256 `0e1449ce44c5f546e87f5de5316c894fb3a1dc0d9bfe87b2ac501eb39602dfd1`。
- 16077 部署结果：`deployment_status=complete`、健康 `200`、Nginx `200`；备份 `20260820-121329-pre-test-20260820-115227-8ee2d198`。16088 未执行部署。
- 认证分发回读：登录 `200`；`agent-bootstrap` `200` 且 `apiUrl=http://192.168.2.145:16077`；Skill ZIP `51,280` bytes、MCP ZIP `15,912` bytes，MCP 包为 26 工具。
- 真实轻协作 API/预览闭环：项目 `mt10grxk7dpq6g`、原型 `mt10grxtgpqnf1`；基线上传入口 `index.html`，源码下载 `6,449` bytes；首次任务兑换成功，重放返回 `400`；候选状态 `ready`，候选预览 `200`；首次采纳生成正式版本 `1`；第二候选采纳返回 `409`，回读状态为 `stale`；正式版本入口仍为 `index.html`，版本列表为 `[1]`，当前原型/项目计数各新增 1 条。
- 旁路健康复核：16077 `/api/health=200`；生产 16088 `/api/health=200`，仅做健康读取，生产未切换。

体验修正 release 与测试环境回读（2026-08-20）：
- 修正 commit：`20ea9192bf49f71bd538f53be27674876829a25d`；release `20260820-142344-20ea9192`；归档 SHA-256 `b19162b6df8f2354dc5603aaf8478c002175bff2eb61530080f6fd467938633b`。
- 16077 部署结果：`deployment_status=complete`、备份 `20260820-142418-pre-test-20260820-142344-20ea9192`、健康 `200`、Nginx `200`；生产 16088 未发布。
- 任务 API 回读：完整提示词包含 `redeem_change_handoff`、`submit_change_candidate` 和原始修改要求；编辑后旧码 `404`、新码可兑换且任务仍为 `editing`；兑换后再次编辑 `409`；删除后任务为 `cancelled`、任务码为 `revoked`。
- 静态前端包回读包含“任务管理器”“复制完整提示词”“伏羲平台”边界文案和任务编辑接口。浏览器登录会话检查发生连接超时，未将其记为视觉通过，待用户在 16077 复测最终视觉效果。
...
- 新增设计包：[agent-runtime-update-mvp](prototypes/agent-runtime-update-mvp/)，冻结“发布 manifest → 伏羲通知 → 用户确认 → 记录 update intent → AI 客户端下次启动 → 稳定 launcher 原子切换 → Smoke/回滚”的纵向切片。

设计冻结边界：
- 服务端拥有 release/artifact/intent 权威状态；MCP 启动回报本地 MCP/Skill/Node 版本。
- 浏览器只负责通知、确认、进度和结果，不直接执行 PowerShell/Node；AI 客户端下次启动时由首次迁移安装的稳定 launcher 承接更新。
