export function buildPrototypePrompt({ requirement, mode, attachmentName }) {
  const strict = mode === 'implementation-proof'
  const attachment = attachmentName
    ? ['- 附件：' + attachmentName + '；先完整读取，再与文字需求合并；冲突时以文字需求为准。']
    : []
  const lines = [
    ...
    '- outputMode: ' + (strict ? 'implementation-proof（严格实现）' : 'alignment（快速验证）') + '。',
  ...
  lines.push(
    '',
    '【需求】',
    requirement || '请完整读取附件中的需求文档，并将其作为唯一业务需求来源。',
    '',
    '【实现要求】',
