<template>
  <div class="usage-dashboard management-page">
    ...
    <section class="management-panel usage-panel retention-panel">
      <div class="usage-panel-head"><div><h2>回访与来源</h2><p>只对已有完整观察窗口的用户计算回访率</p></div><span class="usage-panel-meta">{{ formatNumber(summary.returningUsers) }} 名回访用户</span></div>
...
const stats = ref({ period: { from: '', to: '' }, dataQuality: { isPartial: true, notes: '' }, summary: {}, trend: [], funnel: [], topPrototypes: [], attentionItems: [], eventBreakdown: [], sourceBreakdown: [], activeUsers: [], retention: [] })
...
const summaryCards = computed(() => [
  ...
  { key: 'returningUsers', label: '回访用户', value: formatNumber(summary.value.returningUsers), foot: `${formatNumber(summary.value.averageActionsPerActiveUser)} 次/活跃用户`, tone: 'positive' },
