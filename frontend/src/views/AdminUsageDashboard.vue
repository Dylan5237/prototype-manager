  ...
  <div class="usage-dashboard management-page">
    <div class="management-page-head usage-page-head">
      <div class="management-page-title">
        ...
        <p class="management-description">观察用户从首次使用到原型产出的有效闭环，并直接处理异常事项</p>
      </div>
    ...
    <div class="usage-main-grid">
      ...
      <section class="management-panel usage-panel attention-panel">
        <div class="usage-panel-head"><div><h2>现在需要处理</h2><p>每一项都对应一个管理员动作</p></div><el-tag size="small" :type="stats.attentionItems.length ? 'warning' : 'success'" effect="light">{{ stats.attentionItems.length ? `${stats.attentionItems.length} 项` : '已清空' }}</el-tag></div>
        <div v-if="stats.attentionItems.length" class="attention-list">
          <button v-for="item in stats.attentionItems.slice(0, 4)" :key="`${item.type}-${item.resourceId}-${item.subtitle}`" class="attention-item" @click="openAttention(item)">
            <span :class="['attention-dot', `is-${item.severity}`]"></span><span class="attention-copy"><strong>{{ item.title }}</strong><small>{{ item.subtitle }} · {{ item.action }}</small></span><el-icon><ArrowRight /></el-icon>
          </button>
        </div>
        <el-empty v-else description="暂无待处理事项" :image-size="48" />
      </section>
    </div>
...
const groups = ref([])
const filters = reactive({ period: '30d', groupId: '', role: '' })
const periodOptions = [{ value: '7d', label: '近 7 天' }, { value: '30d', label: '近 30 天' }, { value: '90d', label: '近 90 天' }]
const stats = ref({ period: { from: '', to: '' }, dataQuality: { isPartial: true, notes: '' }, summary: {}, trend: [], funnel: [], topPrototypes: [], attentionItems: [] })
...
const summaryCards = computed(() => [
  ...
  { key: 'pendingItems', label: '待处理事项', value: formatNumber(summary.value.pendingItems), foot: summary.value.pendingItems ? '建议优先处理高风险项' : '当前没有待办', tone: summary.value.pendingItems ? 'warning' : 'positive' }
])
...
function getDateParams() { const days = Number(filters.period.replace('d', '')); const end = new Date(); const start = new Date(end.getTime() - (days - 1) * 24 * 60 * 60 * 1000); return { from: start.toISOString(), to: end.toISOString(), groupId: filters.groupId || undefined, role: filters.role || undefined } }
