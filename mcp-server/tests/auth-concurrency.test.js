...
function createMcpProcess(apiUrl, credentialsFile, extraEnv = {}) {
  const child = spawn(process.execPath, ['src/server.js'], {
    ...
    env: {
      ...
      FUXI_INSTALL_ROOT: path.join(path.dirname(credentialsFile), 'runtime'),
      FUXI_TOKEN: '',
      FUXI_CONNECT_CODE: '',
      FUXI_USERNAME: '',
      FUXI_PASSWORD: '',
...
test('separate MCP processes reuse a just-rotated access token instead of stacking refresh rotations', async () => {
  ...
  try {
    ...
    const first = createMcpProcess(apiUrl, credentialsFile, { FUXI_MCP_ALLOW_MULTI: '1' });
    const second = createMcpProcess(apiUrl, credentialsFile, { FUXI_MCP_ALLOW_MULTI: '1' });
    try {
      await Promise.all([
...
test('cross-process getSessionAuth single-flights refresh and waiters reread credentials', async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'fuxi-session-auth-workers-'));
  let server;
  let refreshCalls = 0;
  let validRefreshToken = 'initial-refresh-token';
  try {
    ...
    const runWorker = () => new Promise((resolve, reject) => {
      const child = spawn(process.execPath, [path.join(__dirname, 'session-auth-worker.js')], {
        env: {
          ...process.env,
          FUXI_API_URL: apiUrl,
          FUXI_CREDENTIALS_FILE: credentialsFile,
          FUXI_TOKEN: '',
          FUXI_DEVICE_LABEL: 'worker'
        },
      ...
      child.on('exit', (code) => {
        try {
          resolve({ code, value: JSON.parse(stdout.trim().split('\n').pop()), stderr });
        } catch (error) {
          reject(new Error(`worker parse failed code=${code} stdout=${stdout} stderr=${stderr}`));
        }
      });
    });
    ...
    assert.equal(refreshCalls, 1);
    const persisted = JSON.parse(fs.readFileSync(credentialsFile, 'utf8'));
    assert.equal(persisted.refreshToken, 'rotated-refresh-token-1');
    assert.equal(typeof persisted.accessToken, 'string');
    assert.equal(first.value.token, persisted.accessToken);
    assert.equal(second.value.token, persisted.accessToken);
  } finally {
    if (server) await new Promise(resolve => server.close(resolve));
    fs.rmSync(root, { recursive: true, force: true });
  }
